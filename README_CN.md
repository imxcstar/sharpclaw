...
- **多渠道架构** — TUI 终端界面（Terminal.Gui v2）+ WebSocket Web 界面 + QQ Bot，通过 `IChatIO` 抽象共享同一套智能体逻辑
- **斜杠命令** — 输入 `/` 触发自动补全，支持 `/exit`、`/quit`、`/config`、`/help`
- **流式输出** — AI 响应实时流式显示，支持推理过程日志和工具调用追踪
- **可配置快捷键** — TUI 退出键、日志切换键、取消键均可自定义
...
配置保存到 `~/.sharpclaw/config.json`。之后启动直接进入交互式对话，输入 `/exit` 或 `/quit` 退出，`Ctrl+Q` 退出程序（默认，可配置）。
...
启动后访问 `http://localhost:5000`（默认）打开 Web 聊天界面。Web UI 支持 Markdown 渲染、代码高亮、连接状态指示和实时状态显示。
...
作为 QQ Bot 服务运行，接收来自 QQ 频道、群聊和私聊的消息。需要在配置文件中配置 QQ Bot 的 AppId 和 ClientSecret。

> 注意：QQ Bot 模式需要先通过 TUI 模式完成配置，并在渠道配置中启用 QQ Bot。

### 其他命令

```bash
dotnet run --project sharpclaw config    # 打开配置界面
dotnet run --project sharpclaw help      # 显示帮助信息
```
...
每个智能体默认继承 `default` 配置，可单独覆盖。设置 `"enabled": false` 可禁用子智能体。渠道配置控制各前端的行为。
...
```
sharpclaw/
├── Program.cs                  # 入口：命令分发（tui/web/qqbot/config/help）
├── Abstractions/               # 接口定义
│   ├── IChatIO.cs              # 前端 I/O 抽象（所有渠道共用）
│   └── IAppLogger.cs           # 日志抽象
├── Agents/                     # 智能体
...
└── wwwroot/
    └── index.html              # Web 聊天界面（单文件 SPA，嵌入资源）
```

## 数据持久化

- `~/.sharpclaw/config.json` — 供应商、智能体和渠道配置
